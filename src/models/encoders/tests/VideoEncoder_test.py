...
class VideoEncoderTest(tf.test.TestCase):
    def testExpectedOutputShape(self):
        ...
        expected_output_shape = (4, (n_frames - window_size) // temporal_stride + 1, embedding_dim)

        video_encoder = VideoEncoder(window_size=window_size, temporal_stride=temporal_stride, embedding_dim=embedding_dim)
