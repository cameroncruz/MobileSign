...
class Decoder(tf.keras.Model):
    def __init__(self, hidden_size, vocab_size, embedding_dim):
        ...
        self.lstm = tf.keras.layers.LSTM(units=hidden_size, return_sequences=False, return_state=True)
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        context = self.attention([self.query(features), self.value(tf.expand_dims(memory_state, 1))])
        x = tf.concat([tf.expand_dims(tf.reduce_sum(context, axis=1), 1), x], axis=-1)
        output, memory_state, carry_state = self.lstm(x, initial_state=[memory_state, carry_state])
