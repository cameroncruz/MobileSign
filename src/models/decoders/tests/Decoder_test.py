...
class DecoderTest(tf.test.TestCase):
    def testExpectedOutputShape(self):
        ...
        decoder = Decoder(hidden_size=hidden_size, vocab_size=vocab_size, embedding_dim=embedding_dim)

        preds, memory_state, carry_state = decoder([tf.ones((batch_size, 1)), tf.ones((batch_size, 5, 100)), tf.ones((batch_size, hidden_size)), tf.ones((batch_size, hidden_size))])
